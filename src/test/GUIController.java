...
import matador.Bonus;
import matador.Brewery;
import matador.Chance;
import matador.Fleet;
import matador.GameTest;
import matador.Jail;
import matador.Ownable;
import matador.Player;
import matador.Start;
import matador.Street;
import matador.TaxProcent;
import matador.VisitJail;
import notusedbyme.*;
...
public class GUIController {	
	...
	public desktop_fields.Field[] createList(matador.Field[] arrayOfFields){
		...
		for(int i = 0; i < list.length; i++){
			...
			list[i].setDescription(arrayOfFields[i].getDescription());
	...
	public void removeOwnerShipFromPlayer(matador.Field[] arrayOfFields, Player player){
		for(int i = 0; i < arrayOfFields.length; i++){
			if(arrayOfFields[i] instanceof matador.Ownable){
	...
	public void updateAllOwnerShip(matador.Field[] arrayOfFields){
		for(int i = 0; i < arrayOfFields.length; i++){
			if(arrayOfFields[i] instanceof matador.Ownable){
